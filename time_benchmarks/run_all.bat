...
for /l %%x in (1, 1, 10) do (
    ...
    gpu\build\Release\HelloOpenCL.exe gpu\string_search_kernel.cl text_files\text%%x.txt text_files\key1.txt results\gpu-text%%x-key1.csv

    @REM test keyword length vs time
    gpu\build\Release\HelloOpenCL.exe gpu\string_search_kernel.cl text_files\text10.txt text_files\key%%x.txt results\gpu-text10-key%%x.csv
